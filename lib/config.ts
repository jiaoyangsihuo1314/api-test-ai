...
export function getExecutorUrl(forClient: boolean = true): string {
  if (forClient) {
    ...
    return process.env.NEXT_PUBLIC_EXECUTOR_URL || 'http://localhost:8012';
  } else {
    // 服务端使用 EXECUTOR_URL（API 路由内部调用）
    return process.env.EXECUTOR_URL || process.env.NEXT_PUBLIC_EXECUTOR_URL || 'http://localhost:8012';
  }
}
